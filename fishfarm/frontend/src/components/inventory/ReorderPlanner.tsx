  ...
  return (
    <div className="bg-white rounded-2xl border p-6">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h3 className="text-lg font-bold text-slate-800">Reorder Planner</h3>
          <p className="text-sm text-slate-500">Suggested purchases based on thresholds</p>
        </div>
      </div>
            ...
            <div key={item.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center p-4 bg-slate-50 rounded-xl border border-slate-100">
              ...
              <div className="flex items-center space-x-4 w-full sm:w-auto justify-between sm:justify-end">
                <div className="text-right">
                  ...
                  <p className="font-medium text-slate-700">{estCost > 0 ? `₹${estCost.toLocaleString()}` : 'Unknown'}</p>
                </div>
                <button
      ...
      <div className="mt-6 pt-4 border-t border-slate-100 flex justify-between items-center">
        <span className="text-slate-600 font-medium">Total Estimated Capital Required</span>
        <div className="flex items-center space-x-1 text-lg font-bold text-slate-800">
