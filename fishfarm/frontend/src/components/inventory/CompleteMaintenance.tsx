  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-slate-800 p-4 rounded-xl border border-slate-700 mb-6">
        <h4 className="font-semibold text-white">{maintenance.maintenanceType}</h4>
        <p className="text-sm text-slate-400 mt-1">Equipment: {maintenance.inventory.itemName}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="md:col-span-2 flex flex-col space-y-3 pt-2">
          <label className="flex items-center space-x-2">
            ...
            />
            <span className="text-sm font-medium text-slate-300">Schedule next maintenance</span>
          </label>
          
          {scheduleNext && (
            <div className="pl-6 animate-in slide-in-from-top-2">
              <label className="block text-sm text-slate-400 mb-1">Next Scheduled Date</label>
              <input
                type="date"
                ...
                className="w-full md:w-1/2 px-4 py-2 border border-slate-700 bg-slate-800 text-white placeholder-slate-500 rounded-xl focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-shadow"
              />
            </div>
          )}
        </div>
