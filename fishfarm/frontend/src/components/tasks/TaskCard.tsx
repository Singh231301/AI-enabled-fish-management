...
export const TaskCard: React.FC<TaskCardProps> = ({ task, onEdit, onComplete, onSkip, viewMode = 'list' }) => {
  ...
  if (viewMode === 'kanban') {
    return (
      <div className={`bg-white rounded-xl shadow-sm border p-4 hover:shadow-md transition-all cursor-pointer ${isOverdue ? 'border-red-300 bg-red-50/30' : 'border-slate-200'}`} onClick={() => onEdit(task)}>
        ...
        <h4 className="font-semibold text-slate-800 mb-1 leading-tight line-clamp-2">{task.title}</h4>
  ...
  // List View
  return (
    <div className={`bg-white rounded-xl shadow-sm border p-4 sm:p-5 hover:shadow-md transition-all ${isOverdue ? 'border-red-200 bg-red-50/20' : 'border-slate-200'}`}>
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            {task.isAiGenerated && (
              <span className="text-xs font-medium px-2.5 py-1 rounded-md flex items-center bg-fuchsia-100 text-fuchsia-700 border border-fuchsia-200">
                <Sparkles className="w-3 h-3 mr-1" /> AI Suggested
              </span>
            )}
            {task.status === 'COMPLETED' && (
              <span className="text-xs font-medium px-2.5 py-1 rounded-md bg-green-100 text-green-700 border border-green-200">
                Done {task.completedDate && format(new Date(task.completedDate), 'MMM d')}
              </span>
            )}
          </div>
          
          <h3 className="text-lg font-bold text-slate-800 mb-1 cursor-pointer hover:text-blue-600 transition-colors" onClick={() => onEdit(task)}>
            {task.title}
          </h3>
          {task.description && (
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-slate-500">
            <div className={`flex items-center font-medium ${isOverdue ? 'text-red-600' : ''}`}>
        ...
        {task.status !== 'COMPLETED' && task.status !== 'SKIPPED' && (
          <div className="flex sm:flex-col gap-2 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 sm:border-l border-slate-100 sm:pl-4">
            <button 
              onClick={() => onComplete(task)}
              className="flex-1 sm:flex-none flex items-center justify-center sm:justify-start px-4 py-2 bg-green-50 text-green-700 hover:bg-green-100 rounded-lg font-medium transition-colors"
            >
            ...
            <button 
              onClick={() => onSkip(task)}
              className="flex-1 sm:flex-none flex items-center justify-center sm:justify-start px-4 py-2 bg-slate-50 text-slate-600 hover:bg-slate-100 rounded-lg font-medium transition-colors"
            >
