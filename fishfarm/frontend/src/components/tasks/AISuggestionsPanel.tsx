    ...
    <div className="bg-gradient-to-br from-fuchsia-50 to-purple-50 rounded-xl border border-fuchsia-100 p-6 mb-8 relative overflow-hidden">
      ...
      <div className="relative z-10">
        <div className="flex items-center mb-4">
          <div className="w-10 h-10 bg-fuchsia-500/20 text-fuchsia-600 rounded-lg flex items-center justify-center mr-3">
            <Sparkles className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-lg font-bold text-white">AI Task Suggestions</h3>
            <p className="text-sm text-slate-300">Based on fish age, season, and water quality context</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          {suggestions.map((suggestion, index) => {
            ...
            return (
              <div key={index} className="bg-slate-800 rounded-lg border border-fuchsia-100 p-4  hover:shadow-md transition-all flex flex-col h-full">
                ...
                <h4 className="font-bold text-white mb-1">{suggestion.title}</h4>
                <p className="text-sm text-slate-300 mb-3 flex-grow line-clamp-3">{suggestion.description}</p>
                
                <div className="flex items-start bg-slate-800/50 rounded p-2 mb-4">
                  <Info className="w-4 h-4 text-slate-400 mt-0.5 mr-2 shrink-0" />
                  <span className="text-xs text-slate-300">{suggestion.reason}</span>
                </div>
