  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">Buyer Name *</label>
          <input 
            type="text" 
            {...register('buyerName')}
            className="w-full rounded-lg bg-slate-800 border-slate-700 text-white shadow-sm focus:border-sky-500 focus:ring-sky-500" 
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">Buyer Phone</label>
          <input 
            type="tel" 
            {...register('buyerPhone')}
        ...
        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">Buyer Location</label>
          <input 
            type="text" 
        ...
        <div className="md:col-span-2 bg-slate-800/50 p-4 rounded-lg border border-slate-800">
          <h4 className="font-semibold text-slate-300 mb-3">Sale Details</h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">Price per Kg (₹) *</label>
              <input 
                type="number" 
                step="0.01"
            ...
            <div className="flex items-center gap-2 md:col-span-2">
              ...
              />
              <label htmlFor="transportIncluded" className="text-sm font-medium text-slate-300">
            ...
            {transportIncluded && (
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-slate-300 mb-1">Transport Cost per Kg (₹)</label>
                <input 
                  type="number" 
                  step="0.01"
        ...
        <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <input 
              ...
              className="w-full rounded-lg bg-slate-700/50 border-slate-700 text-white shadow-sm focus:border-sky-500 focus:ring-sky-500 font-bold" 
            />
            {errors.totalAmount && <p className="text-red-500 text-xs mt-1">{errors.totalAmount.message}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Advance Received (₹)</label>
            <input 
              type="number" 
              step="0.01"
              {...register('advanceReceived', { valueAsNumber: true })}
              className="w-full rounded-lg bg-slate-800 border-slate-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 text-green-400 font-bold" 
            />
            {errors.advanceReceived && <p className="text-red-500 text-xs mt-1">{errors.advanceReceived.message}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">Balance Pending (₹)</label>
            <input 
              type="number" 
              value={balance > 0 ? balance : 0}
              disabled
              className="w-full rounded-lg bg-slate-700/50 border-slate-700 shadow-sm focus:border-sky-500 focus:ring-sky-500 font-bold text-red-400" 
            />
          </div>
        </div>

        <div>
          <label className="block text-sm font-medium text-slate-300 mb-1">Payment Method (for advance)</label>
          <select 
      ...
      <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="px-4 py-2 bg-green-600 text-white font-medium rounded-lg hover:bg-green-500 transition-colors disabled:opacity-50"
        >
