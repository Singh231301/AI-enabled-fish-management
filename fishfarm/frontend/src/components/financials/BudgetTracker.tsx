  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
      <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-slate-50">
        <h3 className="font-bold text-slate-800 text-lg">Monthly Budget Tracker</h3>
        <button 
      ...
      <div className="p-6 space-y-6">
        {totals && (
          <div className="mb-8">
            <div className="flex justify-between items-end mb-2">
              <div>
                <h4 className="font-semibold text-slate-800 text-lg">Overall Budget</h4>
                <p className="text-sm text-slate-500">₹{totals.actual.toLocaleString()} spent of ₹{totals.budgeted.toLocaleString()}</p>
              </div>
              <div className={`font-bold ${totals.status === 'over' ? 'text-red-600' : 'text-green-600'}`}>
        ...
        <div className="space-y-4">
          <h5 className="font-semibold text-slate-700 text-sm uppercase tracking-wider">By Category</h5>
          {categories.map(cat => {
              ...
              <div key={cat.category} className="group">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="text-sm flex items-center gap-2">
                    <span className="font-medium text-slate-800">₹{cat.actual.toLocaleString()}</span>
                    ...
                    <button 
                      ...
                      className="opacity-0 group-hover:opacity-100 text-sky-600 text-xs ml-2 transition-opacity"
                    >
                      Edit
                    </button>
                  </div>
