  ...
  return (
    <div className="bg-slate-900 rounded-xl border border-slate-800 overflow-hidden shadow-sm">
      <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-800/50">
        <h3 className="font-bold text-white text-lg">Break-Even Analysis</h3>
      </div>
      
      <div className="p-6">
        <div className="text-center mb-6">
          <div className="inline-block p-4 rounded-full bg-slate-800/50 border-4 border-slate-800 mb-2">
        ...
        <div className="mb-6">
          <div className="flex justify-between text-sm mb-2">
            ...
            <span className="font-bold text-sky-400">{data.percentageToBreakEven.toFixed(1)}%</span>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 text-center">
          <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-800">
            <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Total Costs</p>
            <p className="font-bold text-white text-lg">₹{data.breakEvenRevenue.toLocaleString()}</p>
          </div>
          <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-800">
            <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Current Revenue</p>
            <p className="font-bold text-green-400 text-lg">₹{data.currentRevenue.toLocaleString()}</p>
          </div>
        </div>
        ...
        {!data.isBreakEvenReached && (
          <div className="mt-4 bg-blue-900/20 border border-blue-500/20 rounded-lg p-4 flex gap-3">
            ...
            <div>
              <p className="text-sm text-blue-400 font-medium">Remaining to Break-Even: <span className="font-bold">₹{data.revenueToBreakEven.toLocaleString()}</span></p>
              {data.breakEvenKg && (
