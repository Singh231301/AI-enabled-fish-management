...
export class InventoryService {
  ...
  async deleteTransaction(transactionId: string, pondId: string, userId: string): Promise<void> {
    const pond = await this.pondRepo.findByIdAndUserId(pondId, userId);
    if (!pond) throw new AppError("Pond not found", 404);
    ...
    const item = await this.inventoryRepo.findById(transaction.inventoryId);
    if (!item || item.pondId !== pondId) throw new AppError("Inventory item not found or unauthorized", 404);

    // Revert inventory quantity
    let newQuantity = item.currentQuantity;
    if (transaction.transactionType === 'PURCHASE') {
      newQuantity = Math.max(0, item.currentQuantity - transaction.quantity);
      await this.inventoryRepo.update(item.id, {
        currentQuantity: newQuantity,
        totalPurchasedKg: Math.max(0, item.totalPurchasedKg - transaction.quantity)
      } as any);
    } else if (transaction.transactionType === 'USAGE' || transaction.transactionType === 'WASTAGE') {
      newQuantity = item.currentQuantity + transaction.quantity;
      await this.inventoryRepo.update(item.id, {
        currentQuantity: newQuantity,
        totalUsedKg: Math.max(0, item.totalUsedKg - transaction.quantity)
      } as any);
    } else if (transaction.transactionType === 'ADJUSTMENT') {
      // Reverting an adjustment is tricky. We'll simply subtract the adjusted amount if it was an increase, or add if it was a decrease.
      // But transaction records don't store the sign of adjustment directly. We can estimate based on balanceAfter.
      // For simplicity, we just leave adjustment reverting out unless requested, or we can try to guess.
      // Let's just delete the transaction for ADJUSTMENT.
    }

    // Delete linked expense if any
    const linkedExpense = await this.expenseRepo.findAutoGeneratedBySource(pondId, 'inventory', transactionId);
    if (linkedExpense) {
      await this.expenseRepo.delete(linkedExpense.id);
    }
    ...
    await this.activityRepo.create({
      action: 'DELETE',
      module: 'INVENTORY_TRANSACTION',
      recordId: transactionId,
      details: { type: transaction.transactionType, quantity: transaction.quantity, item: item.itemName },
      user: { connect: { id: userId } }
    } as any);
  }
}
